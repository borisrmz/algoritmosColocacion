...
namespace ConsoleApplication4
{
    class MejorAjuste : IColocacion
    {
        public DataTable getColocacionEnMemoria(int tPagina, DataTable marcos)
        {
            ...
            int mMenor = marcos.AsEnumerable().Where(m => m["ESTADO"].ToString() == "H" && Convert.ToInt16(m["TAMANIO"]) >= tPagina).Min(m => Convert.ToInt16(m["TAMANIO"]));

            foreach (DataRow marco in marcos.Rows)
            {

                if (tPagina <= Convert.ToInt32(marco["TAMANIO"]) && marco["ESTADO"].ToString() == "H"
                                        && !ingresada && Convert.ToInt16(marco["TAMANIO"]) == mMenor)
                {
                    dr = result.NewRow();
                    dr["ID"] = count;
