...
namespace ConsoleApplication4
{
    class PeorAjuste : IColocacion
    {
        public DataTable getColocacionEnMemoria(int tPagina, DataTable marcos)
        {
            ...
            int mMayor = marcos.AsEnumerable().Where(m => m["ESTADO"].ToString() == "H" && Convert.ToInt16(m["TAMANIO"]) >= tPagina).Max(m => Convert.ToInt16(m["TAMANIO"]));

            foreach (DataRow marco in marcos.Rows)
            {

                if (tPagina <= Convert.ToInt32(marco["TAMANIO"]) && marco["ESTADO"].ToString() == "H"
                                        && !ingresada && Convert.ToInt16(marco["TAMANIO"]) == mMayor)
                {
                    dr = result.NewRow();
                    dr["ID"] = count;
